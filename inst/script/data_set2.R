...
for (i in 1:100) {
  ################# normal distribution ####################
  c1=matrix(rnorm(n1*p1,mean=2,sd=1.8), ncol=p1,nrow=n1)
  c2=matrix(rnorm(n2*p2,mean=1.5, sd=1.8), ncol=p2,nrow=n2)
  c3=matrix(rnorm(n3*p3,mean=1, sd=1.8), ncol=p3,nrow=n3)
  
  normData = matrix(rnorm(n*p, mean=0, sd=1.8),nrow=n, ncol=p)
  ...
  binomData = matrix(rbinom(n*p, size=1, prob=0.15),nrow=n, ncol=p)
  binomData[1:n1,1:p1]= c1
  binomData[(n1+1):(n1+n2),(p1+1):(p1+p2)]= c2
  binomData[(n1+n2+1):n,(p1+p2+1):(p1+p2+p3)]= c3
  
  ######### poisson data ############################
  c1=matrix(rpois(n1*p1,lambda=1.8), ncol=p1,nrow=n1)
  c2=matrix(rpois(n2*p2,lambda=1.4), ncol=p2,nrow=n2)
  ...
  data_set2[[i]]=data_for_klic
}

save(data_set2,file="/home/mercury02/huchuang/wcc/data/data_set2.RData")
