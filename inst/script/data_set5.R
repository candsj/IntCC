...
for (i in 1:100) {
  ################# normal distribution ####################
  c1=matrix(rnorm(n1*p1,mean=1.8,sd=2.4), ncol=p1,nrow=n1)
  c2=matrix(rnorm(n2*p2,mean=1.4, sd=2.4), ncol=p2,nrow=n2)
  c3=matrix(rnorm(n3*p3,mean=1, sd=2.4), ncol=p3,nrow=n3)
  
  normData = matrix(rnorm(n*p, mean=0, sd=2.4),nrow=n, ncol=p)
  ...
  ######## binomial data #########################
  c1=matrix(rbinom(n1*p1,size=1, prob=0.5), ncol=p1,nrow=n1)
  c2=matrix(rbinom(n2*p2,size=1, prob=0.4), ncol=p2,nrow=n2)
  c3=matrix(rbinom(n3*p3,size=1, prob=0.3), ncol=p3,nrow=n3)
  
  binomData = matrix(rbinom(n*p, size=1, prob=0.18),nrow=n, ncol=p)
  ...
  ######### poisson data ############################
  c1=matrix(rpois(n1*p1,lambda=1.6), ncol=p1,nrow=n1)
  c2=matrix(rpois(n2*p2,lambda=1.3), ncol=p2,nrow=n2)
  ...
  c1=matrix(sample(1:3,size=n1*p1, replace=TRUE, prob=c(0.6,0.3,0.1)), ncol=p1,nrow=n1)
  c2=matrix(sample(1:3,size=n2*p2, replace=TRUE, prob=c(0.3,0.4,0.3)), ncol=p2,nrow=n2)
  c3=matrix(sample(1:3,size=n3*p3, replace=TRUE, prob=c(0.1,0.3,0.6)), ncol=p3,nrow=n3)
  ...
  data_set5[[i]]=data_for_klic
  
}

save(data_set5,file="/home/mercury02/huchuang/wcc/data/data_set5.RData")
